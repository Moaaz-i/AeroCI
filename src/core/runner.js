...
class Runner {
    static run(workflowPath = '.github/workflows/main.yml', options = {}) {
        let targetFile = path.resolve(process.cwd(), workflowPath);

        // Auto discovery if default path doesn't exist
        ...
        if (!fs.existsSync(targetFile)) {
            Logger.error(`Workflow file not found at: ${workflowPath}`);
        ...
        if (fs.existsSync(envFilePath)) {
            ...
            Logger.info(`Loaded local environment variables from .env`);
        }
        ...
        if (failedSteps === 0) {
            Logger.success(`All simulated pipeline steps completed successfully! (0 remote minutes wasted). ✨`);
        } else {
            Logger.error(`Pipeline simulation finished with ${failedSteps} failed step(s).`);
        }
    }
}
